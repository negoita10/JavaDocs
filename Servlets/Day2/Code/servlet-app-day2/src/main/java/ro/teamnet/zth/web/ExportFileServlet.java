...
//        Arrays.asList(request.getParameterNames()).stream().map(name->paramsMap.put(name,request.getParameter(String.valueOf(name))));

        Collections.list(request.getParameterNames()).forEach(name->paramsMap.put(name,request.getParameter(String.valueOf(name))));
        ...
        try (InputStream reportStream = getClass().getResourceAsStream(jasperPath)) {
            ...
            response.setHeader("Content-Language", "Content-Disposition : attachement" + paramsMap);
